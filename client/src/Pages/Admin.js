...
function Admin() {
  const [users, setUsers] = useState([]);
  const [newUser, setNewUser] = useState({ name: '', email: '', role: '', verified: false });
  const [editUser, setEditUser] = useState(null);
  ...
  const handleCreateUser = async () => {
    ...
    const newUserWithId = await response.json();
    setUsers([...users, newUserWithId]);
    setNewUser({ name: '', email: '', role: '', verified: false });
  };
          ...
          <tbody>
            {users.map((user) => (
              <tr key={user._id}>
                ...
                <td>
                  {!user.verified && (
                    <button style={{ backgroundColor: 'blue' }} onClick={() => handleVerifyUser(user._id)}>Verify</button>
                  )}
                </td>
              </tr>
            ))}
